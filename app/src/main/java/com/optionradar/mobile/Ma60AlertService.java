...
public class Ma60AlertService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        startForeground(MONITOR_NOTIFICATION_ID, buildMonitorNotification("1分K MA60 監看中"));
        acquireWakeLock();
        startWorkerIfNeeded();
        return START_STICKY;
    }
        ...
        worker = new Thread(() -> {
            while (running) {
                try {
                    ...
                    // Network/Tailscale outages are expected occasionally. Keep polling.
                }
                try {
    ...
    private void pollOnce() throws Exception {
        ...
        try {
            conn = (HttpURLConnection) new URL(base + "/api/ma60-alert").openConnection();
            ...
            if (age > MAX_EVENT_AGE_SEC) return; // Never pop an old crossing after reconnect/reboot.
