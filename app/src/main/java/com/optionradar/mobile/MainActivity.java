...
public class MainActivity extends Activity {
    ...
    private View buildUi() {
        ...
        Button radarSettings = new Button(this);
        radarSettings.setText("雷達設定（同步 PC）");
        radarSettings.setTextColor(C_TEXT);
        radarSettings.setTextSize(13);
        radarSettings.setAllCaps(false);
        radarSettings.setBackground(roundBg(Color.rgb(28, 48, 75), Color.rgb(71, 101, 143), 10));
        radarSettings.setOnClickListener(v -> loadAndShowRadarSettings());
        LinearLayout.LayoutParams radarSettingsLp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(42));
        radarSettingsLp.topMargin = dp(8);
        page.addView(radarSettings, radarSettingsLp);
        ...
        // v0.6 TAIEX estimated turnover research + mobile radar settings
        ...
        TextView footer = text("OptionRadar Android v0.6｜K線 + 加權估量 + 雷達設定", 11, C_MUTED, false);
    ...
    private static final String[][] RADAR_CFG = new String[][]{
            {"radar_mode","雷達模式 (AUTO / 0DTE / GENERAL)"},{"radar_start","0DTE 開始時間"},{"radar_end","0DTE 結束時間"},
            {"max_ask","0DTE 最高 Ask"},{"min_ask","0DTE 最低 Ask"},{"preferred_low","優先權利金下限"},{"preferred_high","優先權利金上限"},
            {"top_n","顯示前 N 名"},{"scan_range_points","掃描範圍(點)"},{"scan_each_side","每邊履約價數"},{"resubscribe_move_points","移動重掃(點)"},
            {"signal_threshold","訊號門檻"},{"signal_gap","多空最小差"},{"general_radar_start","一般開始時間"},{"general_radar_end","一般結束時間"},
            {"general_max_ask","一般最高 Ask"},{"general_min_ask","一般最低 Ask"},{"general_preferred_low","一般優先下限"},{"general_preferred_high","一般優先上限"},
            {"general_signal_threshold","一般訊號門檻"},{"general_signal_gap","一般多空最小差"},{"general_scan_range_points","一般掃描範圍"}
    };

    private void loadAndShowRadarSettings() {
        Toast.makeText(this, "正在讀取 PC 實際設定…", Toast.LENGTH_SHORT).show();
        net.execute(() -> {
            try {
                JSONObject cfg = httpJson("/api/mobile/config", null);
                ui.post(() -> showRadarSettingsDialog(cfg));
            } catch (Exception e) {
                ui.post(() -> Toast.makeText(this, "雷達設定讀取失敗：" + e.getMessage(), Toast.LENGTH_LONG).show());
            }
        });
    }

    private void showRadarSettingsDialog(JSONObject cfg) {
        ScrollView scroll = new ScrollView(this);
        LinearLayout box = new LinearLayout(this);
        box.setOrientation(LinearLayout.VERTICAL);
        box.setPadding(dp(18), dp(8), dp(18), dp(8));
        scroll.addView(box);
        final Map<String, EditText> edits = new HashMap<>();
        for (String[] d : RADAR_CFG) {
            TextView lab = text(d[1], 12, C_MUTED, false);
            lab.setPadding(0, dp(7), 0, dp(2));
            EditText e = new EditText(this);
            e.setSingleLine(true);
            e.setText(cfg.optString(d[0], ""));
            e.setTextColor(C_TEXT);
            e.setHintTextColor(C_MUTED);
            e.setBackgroundColor(C_PANEL2);
            e.setPadding(dp(9), dp(6), dp(9), dp(6));
            box.addView(lab);
            box.addView(e, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(42)));
            edits.put(d[0], e);
        }
        TextView note = text("儲存後直接寫入 PC v1.63 Runtime 與 config.json；重新開啟會再從 PC 讀取實際值。", 11, C_AMBER, false);
        note.setPadding(0, dp(12), 0, dp(4)); box.addView(note);
        AlertDialog dlg = new AlertDialog.Builder(this).setTitle("雷達設定｜PC 同步")
                .setView(scroll).setNegativeButton("取消", null).setPositiveButton("儲存並立即套用", null).create();
        dlg.setOnShowListener(x -> dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            JSONObject out = new JSONObject();
            try {
                for (String[] d : RADAR_CFG) {
                    String key=d[0], val=edits.get(key).getText().toString().trim();
                    if (key.contains("start") || key.contains("end") || "radar_mode".equals(key)) out.put(key,val);
                    else out.put(key, val.isEmpty() ? 0 : Double.parseDouble(val));
                }
            } catch (Exception ex) { Toast.makeText(this,"數值格式錯誤："+ex.getMessage(),Toast.LENGTH_LONG).show(); return; }
            dlg.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(false);
            net.execute(() -> {
                try {
                    JSONObject r=httpJson("/api/mobile/config",out);
                    boolean ok=r.optBoolean("ok",false); String msg=r.optString("message", ok?"已生效":"設定失敗");
                    ui.post(() -> { Toast.makeText(this, ok?"已同步至 PC／已生效":"設定失敗："+msg,Toast.LENGTH_LONG).show(); if(ok) dlg.dismiss(); else dlg.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(true); });
                } catch(Exception ex) { ui.post(() -> { Toast.makeText(this,"設定寫入失敗："+ex.getMessage(),Toast.LENGTH_LONG).show(); dlg.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(true); }); }
            });
        }));
        dlg.show();
    }

    private JSONObject httpJson(String path, JSONObject post) throws Exception {
        String base=serverUrl==null?"":serverUrl.trim(); while(base.endsWith("/")) base=base.substring(0,base.length()-1);
        HttpURLConnection c=(HttpURLConnection)new URL(base+path+(post==null?"?ts="+System.currentTimeMillis():"")).openConnection();
        c.setConnectTimeout(3500); c.setReadTimeout(5000); c.setUseCaches(false);
        if(post!=null){ c.setRequestMethod("POST"); c.setDoOutput(true); c.setRequestProperty("Content-Type","application/json; charset=UTF-8"); byte[] b=post.toString().getBytes("UTF-8"); c.setFixedLengthStreamingMode(b.length); try(OutputStream os=c.getOutputStream()){os.write(b);} }
        int code=c.getResponseCode(); if(code<200||code>=300) throw new Exception("HTTP "+code);
        BufferedReader br=new BufferedReader(new InputStreamReader(c.getInputStream(),"UTF-8")); StringBuilder sb=new StringBuilder(); String line; while((line=br.readLine())!=null)sb.append(line); br.close(); c.disconnect();
